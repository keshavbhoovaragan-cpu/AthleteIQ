...
def get_player_from_db(player_id: int):
    ...
    conn.close()
    if row and rows: return dict(row), [dict(r) for r in rows]
    return None, []

def trade_recommendation(give_fs, receive_fs, give_pred, receive_pred):
    diff = receive_fs - give_fs
    pred_diff = (receive_pred or receive_fs) - (give_pred or give_fs)
    if diff > 8 and pred_diff > 5:
        return "Strong Accept","You're receiving a significantly better player. Make this trade.","#22c55e"
    elif diff > 3:
        return "Accept","The player you're receiving has the edge in fantasy value.","#86efac"
    elif diff < -8 and pred_diff < -5:
        return "Strong Decline","You're giving away a much better player. Don't make this trade.","#ef4444"
    elif diff < -3:
        return "Decline","You're giving away the better player. You'd be losing value.","#fca5a5"
    else:
...
async def analyze_trade(player_a_id: int, player_b_id: int):
    ...
    if not a_latest or not b_latest:
        return {"error": "Could not fetch stats. Make sure both players are in our database."}
    ...
    return {
        "player_a":{**a_latest,"fantasy_score":a_fs,"grade":grade(a_fs),"prediction":a_pred,"total_seasons":len(a_seasons)},
        "player_b":{**b_latest,"fantasy_score":b_fs,"grade":grade(b_fs),"prediction":b_pred,"total_seasons":len(b_seasons)},
        "verdict":verdict,"reason":reason,"color":color,
        "category_wins":{"a":a_wins,"b":len(cats)-a_wins},
        "value_diff":round(b_fs-a_fs,1)
    }
